...
export default function Hero() {
 ...
 const [statsRef, statsInView] = useInView(0.3)
  ...
  return (
    <section className="hero min-h-[92vh] py-20 px-6 relative overflow-hidden flex items-center bg-[radial-gradient(ellipse_at_70%_30%,#e0d6ff_0%,#f0eeff_35%,#ffffff_70%)] bg-size-[200%_200%] animate-[gradShift_10s_ease_infinite]">
      {/* Grid overlay */}
      ...
      <div className="hero-inner max-w-7xl mx-auto w-full grid grid-cols-1 lg:grid-cols-2 gap-15 items-center relative z-10">
        {/* Left column */}
        <div>
          ...
          <div className="hero-ctas flex gap-3 flex-wrap animate-[fadeUp_0.8s_ease_both_0.45s]">
            <button className="btn-p px-6 py-3 bg-linear-to-r from-[#7c3aed] to-[#6d28d9] text-white rounded-[10px] font-bold inline-flex items-center gap-2 hover:-translate-y-0.5 hover:shadow-xl transition">
              <svg width="15" height="15" viewBox="0 0 24 24" fill="white">
                <path d="M15.5 14h-.79l-.28-.27A6.47 6.47 0 0 0 16 9.5 6.5 6.5 0 1 0 9.5 16c1.61 0 3.09-.59 4.23-1.57l.27.28v.79l5 4.99L20.49 19l-4.99-5zm-6 0C7.01 14 5 11.99 5 9.5S7.01 5 9.5 5 14 7.01 14 9.5 11.99 14 9.5 14z" />
              </svg>
              Explore Properties
            </button>
            <button className="btn-ghost px-6 py-3 bg-[rgba(124,58,237,0.06)] border border-[rgba(124,58,237,0.3)] rounded-[10px] text-[#1a0a2e] font-semibold hover:bg-[rgba(124,58,237,0.12)] hover:-translate-y-0.5 transition">
              Post Property Free
            </button>
          </div>

          <div className="hero-stats flex gap-8 mt-10 animate-[fadeUp_0.8s_ease_both_0.55s]">
            <div>
              <div className="hstat-num font-serif text-3xl font-extrabold text-[#1a0a2e] leading-none" id="c1">
                0<span className="bg-linear-to-r from-[#7c3aed] to-[#a78bfa] bg-clip-text text-transparent">+</span>
              </div>
        ...
        <div>
          <div className="sc bg-white border border-[rgba(124,58,237,0.2)] rounded-[20px] p-6 shadow-2xl shadow-[rgba(124,58,237,0.15)] animate-[slideL_0.9s_cubic-bezier(0.34,1.2,0.64,1)_both_0.3s]">
            <div className="sc-inner animate-[floatY_5s_ease-in-out_infinite]">
              ...
              {/* Tabs */}
              <div className="sc-tabs flex gap-1 mb-4" id="sc-tabs">
                {['Buy', 'Rent', 'PG', 'Plot', 'Commercial'].map((tab, i) => (
                  <button
                    key={tab}
                    className={`sc-tab flex-1 py-2 px-1.5 rounded-lg border text-xs font-semibold transition-all text-center ${
                      i === 0
              ...
              <div className="sc-mb mb-3">
                <label className="sc-lbl block text-[10.5px] font-bold uppercase tracking-wide text-[rgba(26,10,46,0.4)] mb-1">City</label>
                <select className="sc-inp w-full h-11 bg-white border border-[rgba(124,58,237,0.2)] rounded-[10px] px-3 text-sm text-[#1a0a2e] outline-none focus:border-[#7c3aed] focus:bg-[rgba(124,58,237,0.03)] transition" id="city-sel" defaultValue="Mumbai">
              ...
              {/* Row: Type & BHK */}
              <div className="sc-row grid grid-cols-2 gap-2.5 mb-3">
                <div>
                  <label className="sc-lbl block text-[10.5px] font-bold uppercase tracking-wide text-[rgba(26,10,46,0.4)] mb-1">Type</label>
                  <select className="sc-inp w-full h-11 bg-white border border-[rgba(124,58,237,0.2)] rounded-[10px] px-3 text-sm text-[#1a0a2e] outline-none">
              ...
              {/* Search button */}
              <button className="btn-p w-full h-12 bg-linear-to-r from-[#7c3aed] to-[#6d28d9] text-white rounded-[10px] font-bold flex items-center justify-center gap-2 text-base hover:-translate-y-0.5 hover:shadow-xl transition" id="search-btn">
                <svg width="15" height="15" viewBox="0 0 24 24" fill="white">
                  <path d="M15.5 14h-.79l-.28-.27A6.47 6.47 0 0 0 16 9.5 6.5 6.5 0 1 0 9.5 16c1.61 0 3.09-.59 4.23-1.57l.27.28v.79l5 4.99L20.49 19l-4.99-5zm-6 0C7.01 14 5 11.99 5 9.5S7.01 5 9.5 5 14 7.01 14 9.5 11.99 14 9.5 14z" />
                </svg>
                <span id="search-btn-txt">Search Mumbai Properties</span>
              </button>
              ...
              <div className="pop-row flex items-center gap-1.5 flex-wrap mt-3">
                ...
                {['Mumbai', 'Bangalore', 'Pune', 'Hyderabad'].map(city => (
                  <button
                    ...
                    onClick={() => {
                      document.getElementById('city-sel').value = city
                      document.getElementById('search-btn-txt').textContent = `Search ${city} Properties`
                    }}
                  >
                    {city}
                  </button>
